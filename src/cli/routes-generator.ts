...
interface DalilaRoutesConfig {
  routeTags?: Partial<RouteTagPolicy>;
}
...
const RESERVED_FILES: Record<string, RouteFileType> = {
...
function getFileType(filename: string): RouteFileType | null {
...
function sanitizeImportName(relPath: string): string {
  return relPath
    .replace(/\\/g, '_')
    .replace(/\//g, '_')
    .replace(/\.(ts|html)$/, '')
...
function routeDataExpr(dataVar?: string): string {
...
function segmentSortRank(segment: string): number {
...
function toImportPath(outputPath: string, sourcePath: string): string {
...
function joinRoutePaths(parent: string, child: string): string {
  if (!child || child === '.') return normalizeRoutePath(parent || '/');
...
interface RouteParamSegment {
  key: string;
  isCatchAll: boolean;
  isOptionalCatchAll: boolean;
}

function parseRouteParamSegment(segment: string): RouteParamSegment | null {
...
function computeRouteScore(routePattern: string): number {
  const segments = normalizeRoutePath(routePattern).split('/').filter(Boolean);
  if (segments.length === 0) return 1_000;
...
function routeIdFromFsPath(fsPath: string): string {
  if (!fsPath) return 'root';
  return fsPath
    .replace(/\\/g, '/')
    .replace(/\//g, '_')
    .replace(/[^a-zA-Z0-9_]/g, '_');
...
function extractTagsFromTsSource(source: string): string[] {
...
function extractHtmlPathFromTsSource(source: string): string | undefined {
  const htmlPathDecl = source.match(/export\s+const\s+htmlPath\s*=\s*(['"`])([^'"`]+)\1/m);
  if (!htmlPathDecl) return undefined;
...
function extractNamedExportsFromTsSource(source: string): string[] {
  ...
  let exportListMatch: RegExpExecArray | null = null;
...
function collectNodeTags(node: RouteNode): string[] {
  const tags = new Set<string>();
  for (const file of node.files) {
    if (file.isHtml || !file.tags) continue;
...
function hasNamedExport(file: RouteFile | undefined, exportName: string): boolean {
...
export async function injectHtmlPathTemplates(node: RouteNode, routesDir: string, projectRoot: string): Promise<void> {
  ...
  for (const file of node.files) {
    if (file.isHtml || !file.htmlPath) continue;

    const hasLocalHtmlSibling = node.files.some(
      (candidate) => candidate.isHtml && candidate.type === file.type
    );
    if (hasLocalHtmlSibling) continue;

    const resolvedHtmlPath = resolveHtmlPath(file.htmlPath, routesDir, file.path, projectRoot);
    if (!await pathExists(resolvedHtmlPath)) {
      throw new Error(
        `htmlPath not found for "${file.path}": "${file.htmlPath}" (resolved to "${resolvedHtmlPath}")`
      );
    }
  ...
  await Promise.all(
    node.children.map(child => injectHtmlPathTemplates(child, routesDir, projectRoot))
  );
}

function hasPageRouteExports(pageTs?: RouteFile): boolean {
  return Boolean(
    pageTs
    && (
...
const DEFAULT_ROUTE_TAG_POLICY: RouteTagPolicy = {
  allowed: ['auth', 'public'],
  exclusiveGroups: [
    ['public', 'auth']
  ],
...
function normalizeStringMatrix(input: unknown, field: string): string[][] {
  if (!Array.isArray(input)) {
    throw new Error(`Invalid route tags config: "${field}" must be an array of string arrays.`);
  }

  return input.map((row, index) =>
    normalizeStringArray(row, `${field}[${index}]`)
  );
}

function mergeTagPolicy(base: RouteTagPolicy, raw?: Partial<RouteTagPolicy>): RouteTagPolicy {
  if (!raw) return base;

  const merged: RouteTagPolicy = {
...
async function loadTagPolicy(routesDir: string, outputPath: string): Promise<RouteTagPolicy> {
  ...
  for (const configPath of candidates) {
    ...
    const parsed = JSON.parse(rawText) as DalilaRoutesConfig | Partial<RouteTagPolicy>;

    const routeTags = (parsed as DalilaRoutesConfig).routeTags
      ? (parsed as DalilaRoutesConfig).routeTags
      : (parsed as Partial<RouteTagPolicy>);
...
function resolvePrimaryTag(tags: string[], policy: RouteTagPolicy): string | undefined {
...
function validateManifestTags(entries: ManifestEntry[], policy: RouteTagPolicy): void {
  for (const entry of entries) {
    ...
    if (policy.allowed) {
      for (const tag of tagSet) {
        if (!policy.allowed.includes(tag)) {
          throw new Error(
            `Route "${entry.pattern}" uses unknown tag "${tag}". Allowed tags: ${policy.allowed.join(', ')}.`
          );
        }
      }
    }
    ...
    for (const group of policy.exclusiveGroups) {
      ...
      if (present.length > 1) {
        throw new Error(
          `Route "${entry.pattern}" has conflicting exclusive tags: ${present.join(', ')}.`
        );
      }
    }
    ...
    for (const pair of policy.forbiddenPairs) {
      ...
      if (tagSet.has(left) && tagSet.has(right)) {
        throw new Error(
          `Route "${entry.pattern}" has forbidden tag combination: ${left} + ${right}.`
        );
      }
    }
  }
...
function stateProps(errorHtml?: RouteFile, errorTs?: RouteFile, pendingHtml?: RouteFile, pendingTs?: RouteFile, notFoundHtml?: RouteFile, notFoundTs?: RouteFile): string[] {
  const props: string[] = [];
...
function pageProps(pageHtml?: RouteFile, pageTs?: RouteFile): string[] {
  const props: string[] = [];
...
function layoutProps(indent: string, layoutHtml?: RouteFile, layoutTs?: RouteFile): string[] {
  const lines: string[] = [];
...
function buildRouteTreeSync(routesDir: string, currentPath = '', currentSegment = ''): RouteNode {
...
export async function buildRouteTree(routesDir: string, currentPath = '', currentSegment = ''): Promise<RouteNode> {
  ...
  for (const entry of entries) {
    ...
    fileReads.push(
      fsp.readFile(fullFilePath, 'utf-8').then(content => {
  ...
  const childBuilds: Promise<RouteNode>[] = [];
  for (const entry of entries) {
    if (!entry.isDirectory()) continue;
    if (entry.name.startsWith('.') || entry.name === 'node_modules') continue;
...
function markLazyModules(node: RouteNode): void {
...
function collectHtmlPathDependencyDirsFromTree(
  node: RouteNode,
  routesDir: string,
  projectRoot: string,
  out: Set<string>
...
export function collectHtmlPathDependencyDirs(routesDir: string): string[] {
...
function generateImports(node: RouteNode, routesDir: string, outputPath: string): string[] {
  const imports: string[] = [];

  for (const file of node.files) {
    if (file.isHtml || file.lazy) continue;
...
function generateLazyModuleLoaders(node: RouteNode, routesDir: string, outputPath: string): string[] {
...
function collectHtmlFiles(node: RouteNode): RouteFile[] {
...
function generateHtmlConstants(htmlFiles: RouteFile[]): string[] {
...
function collectManifestEntries(
  ...
  for (const child of [...node.children].sort(compareRouteNodes)) {
    entries.push(...collectManifestEntries(
      child,
      routesDir,
      outputPath,
      currentPattern,
...
function validateManifestEntries(entries: ManifestEntry[]): void {
  const seenByPattern = new Map<string, string>();
  const seenById = new Map<string, string>();
  for (const entry of entries) {
    const existingPattern = seenByPattern.get(entry.pattern);
    if (existingPattern) {
      throw new Error(
        `Conflicting routes for pattern "${entry.pattern}": "${existingPattern}" and "${entry.id}".`
      );
    }
    ...
    if (existingId) {
      throw new Error(
        `Duplicate route ID "${entry.id}" generated for patterns "${existingId}" and "${entry.pattern}". ` +
...
function generateManifestFile(entries: ManifestEntry[], policy: RouteTagPolicy): string {
...
function paramsTypeForPattern(pattern: string): string {
...
function generateRouteTypesFile(entries: ManifestEntry[]): string {
...
function generateRouteObject(node: RouteNode, depth = 0): string | null {
 ...
 */
export async function generateRoutesFile(routesDir: string, outputPath: string): Promise<void> {
