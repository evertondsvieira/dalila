...
function extractTemplateIdentifiers(html: string): TemplateIdentifier[] {
  ...
  const DIRECTIVE_RE =
    /\b(d-each|d-virtual-each|d-virtual-height|d-virtual-item-height|d-virtual-estimated-height|d-virtual-measure|d-virtual-infinite|d-virtual-overscan|d-if|d-when|d-match|d-portal|d-html|d-attr-[a-zA-Z][\w-]*|d-bind-[a-zA-Z][\w-]*|d-on-[a-zA-Z][\w-]*|d-form-error|d-form|d-array)\s*=\s*(['"])([\s\S]*?)\2/g;
  ...
  while ((match = DIRECTIVE_RE.exec(html))) {
    ...
    if (!value) continue;
    if (directive === 'd-virtual-measure' && value.toLowerCase() === 'auto') continue;
