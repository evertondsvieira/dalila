...
let profilerWindowMs = 10_000;
let profilerSamplesPerNode = 128;
const profilerSamplesByNode = new Map<number, Array<{ at: number; durationMs: number }>>();
const profilerRunBucketsByNode = new Map<number, Array<{ at: number; count: number }>>();
let runStartStacks = new WeakMap<object, number[]>();
...
function markRunStart(ref: object): void {
  markRun(ref);
  if (!enabled || !profilerEnabled) return;
...
function pruneProfilerSamples(nodeId: number, nowAt: number): Array<{ at: number; durationMs: number }> {
  const samples = profilerSamplesByNode.get(nodeId) ?? [];
  if (samples.length === 0) return samples;

  const minAt = nowAt - profilerWindowMs;
  let start = 0;
  while (start < samples.length && samples[start].at < minAt) start++;
  ...
  if (next !== samples) profilerSamplesByNode.set(nodeId, next);
  return next;
}

function pruneProfilerRunBuckets(nodeId: number, nowAt: number): Array<{ at: number; count: number }> {
  const buckets = profilerRunBucketsByNode.get(nodeId) ?? [];
  if (buckets.length === 0) return buckets;

  const minAt = nowAt - profilerWindowMs;
  let start = 0;
  while (start < buckets.length && buckets[start].at < minAt) start++;
  const next = start > 0 ? buckets.slice(start) : buckets;
  if (next !== buckets) profilerRunBucketsByNode.set(nodeId, next);
  return next;
}

function markRunEnd(ref: object): void {
  if (!enabled || !profilerEnabled) return;

  const stack = runStartStacks.get(ref);
  if (!stack || stack.length === 0) return;
  const start = stack.pop()!;
  if (stack.length === 0) runStartStacks.delete(ref);
...
function summarizeProfilerNode(nodeId: number): DevtoolsProfilerNodeSample | null {
  const now = Date.now();
  const runBuckets = pruneProfilerRunBuckets(nodeId, now);
  const samples = pruneProfilerSamples(nodeId, now);
  if (samples.length === 0 && runBuckets.length === 0) return null;
...
export function getProfilerSnapshot(): DevtoolsProfilerSnapshot {
  const profiledNodes: DevtoolsProfilerNodeSample[] = [];
  for (const node of nodes.values()) {
    if (node.type !== "effect" && node.type !== "effectAsync" && node.type !== "computed") continue;
    const summary = summarizeProfilerNode(node.id);
    if (summary) profiledNodes.push(summary);
  }
...
export function setProfilerEnabled(
  next: boolean,
  options?: { windowMs?: number; samplesPerNode?: number }
): void {
  if (typeof options?.windowMs === "number" && Number.isFinite(options.windowMs)) {
    const nextWindowMs = Math.floor(options.windowMs);
    if (nextWindowMs > 0) profilerWindowMs = nextWindowMs;
  }
  if (
    typeof options?.samplesPerNode === "number" &&
    Number.isFinite(options.samplesPerNode)
  ) {
    const nextSamplesPerNode = Math.floor(options.samplesPerNode);
    if (nextSamplesPerNode > 0) profilerSamplesPerNode = nextSamplesPerNode;
  }
  ...
  if (!next) {
    ...
    runStartStacks = new WeakMap<object, number[]>();
  }
  if (enabled) {
...
export function trackEffectRunStart(effectRef: object): void {
  markRunStart(effectRef);
}

export function trackEffectRunEnd(effectRef: object): void {
  markRunEnd(effectRef);
}

export function trackComputedRunStart(computedRef: object): void {
  markRunStart(computedRef);
}

export function trackComputedRunEnd(computedRef: object): void {
