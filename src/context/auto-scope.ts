...
import { getCurrentScope, createScope, isScopeDisposed, withScope, type Scope } from '../core/scope.js';
import { isInDevMode } from '../core/dev.js';
import { signal as createSignal, type Signal } from '../core/signal.js';
import {
  createContext,
  provide as rawProvide,
  tryInject as rawTryInject,
 ...
 */
const globalStorage = globalThis as unknown as { [DALILA_GLOBAL_SCOPE_KEY]?: Scope | null };
 ...
 */
function getGlobalRootScope(): Scope | null {
  return globalStorage[DALILA_GLOBAL_SCOPE_KEY] ?? null;
}

function setGlobalRootScope(scope: Scope | null): void {
...
let autoScopePolicy: AutoScopePolicy = "throw";

export function setAutoScopePolicy(policy: AutoScopePolicy): void {
  autoScopePolicy = policy;
}

function warnGlobalProvideOnce(): void {
  if (!isInDevMode()) return;
  if (warnedGlobalProvide) return;
  warnedGlobalProvide = true;

  console.warn(
    '[Dalila] provide() called outside a scope. Using a global root scope (auto-scope). ' +
      'Prefer scope(() => { ... }) or provideGlobal() for explicit globals. ' +
      'You can also setAutoScopePolicy("throw") to prevent accidental globals.'
  );
}

function detachBeforeUnloadListener(): void {
...
function getOrCreateGlobalScope(): Scope {
...
export function provide<T>(token: ContextToken<T>, value: T): void {
  ...
  if (currentScope) {
    ...
    if (autoScopePolicy === "throw") {
      throw new Error(
        "[Dalila] provide() called outside a scope. " +
 ...
 * - Never warns.
 */
export function provideGlobal<T>(token: ContextToken<T>, value: T): void {
  const globalScope = getOrCreateGlobalScope();
  withScope(globalScope, () => {
    rawProvide(token, value);
  });
 ...
 */
export function injectGlobal<T>(token: ContextToken<T>): T {
  let globalRootScope = getGlobalRootScope();
  ...
  if (!globalRootScope) {
    ...
    throw new Error(
      "[Dalila] injectGlobal() called but no global scope exists yet. " +
  ...
  return withScope(globalRootScope, () => {
    ...
    if (result.found) return result.value;
    throw new Error(
      "[Dalila] injectGlobal() token not found in global scope. " +
 ...
 */
function createContextNotFoundError(token: ContextToken<any>): string {
...
function createInjectOutsideScopeError(token: ContextToken<any>, extra?: string): Error {
  const name = token.name || 'unnamed';
  return new Error(
    `[Dalila] Context '${name}' not found.\n\n` +
      (extra ? `${extra}\n` : '') +
 ...
 */
export function getGlobalScope(): Scope | null {
 ...
 */
export function resetGlobalScope(): void {
  detachBeforeUnloadListener();
...
export interface SignalContext<T> {
   ...
   */
  tryInject: () =>
    | { found: true; signal: Signal<T> }
    | { found: false; signal: undefined };
 ...
 */
export function createSignalContext<T>(name: string, defaultValue: T): SignalContext<T> {
  const token = createContext<Signal<T>>(name);

  return {
    provide(initialValue?: T): Signal<T> {
      const sig = createSignal<T>(initialValue !== undefined ? initialValue : defaultValue);
      provide(token, sig);
      return sig;
    },
